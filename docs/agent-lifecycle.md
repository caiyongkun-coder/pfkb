...
- `human-review.html`：静态备选批复页面，可导出 `review-decisions.jsonl`；日常优先用 `review-server` 服务页直接写回。
- `asset-index.jsonl`：批复动作应用后的最终资产索引，适合 agent 优先读取。

### 3. 人类批复，Agent 继续

这里的关键设计是：服务模式默认负责人类批复并直接写回，agent 负责继续流程；静态页面只作为服务无法启动时的备选。

推荐流程：

```text
agent 生成 human-review.jsonl
-> agent 启动 review-server 并打开 review_url
-> 人类在服务页批复并提交
-> 服务写回 review-decisions.jsonl / next-actions.jsonl
-> agent 根据批复继续执行
```

静态 HTML 页面不需要自己启动本地命令，只用于无法启动服务时的手动导出流程。
...
```json
{
  ...
  "status": "approved",
  "note": "可以让当前宿主 agent 大模型读取已提取文本复核，不允许上传源文件。",
...
当前实现已经能把这些动作落成计划文件，并应用到资产索引里的 `asset_status`、`review_action`、`manual_tags` 等字段，不会直接修改源文件或隐私配置。复核页的模型复核会进入 `queue_agent_semantic_review`，由宿主 agent 读取本地 `extracted_text_path` 后写回；显式 `cloud-llm` 模式仍必须走独立配置授权和风险确认，不是复核页默认动作。
...
服务模式 `review-server` 是默认体验：页面提交后直接写回本地批复文件，并在标准 run 目录下刷新资产索引。静态 `human-review.html` 仍可完成批复，但它不会自己启动后续命令，只在服务无法启动时使用。

合理职责是：

```text
review-server 写回 review-decisions.jsonl / next-actions.jsonl
agent 读取 next-actions.jsonl 并继续
```
...
1. 让 agent 处理 `next-actions.jsonl` 中的 `queue_agent_semantic_review`，读取已提取文本完成语义复核，并把结果再次回写资产索引。
