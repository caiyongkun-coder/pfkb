...
`pfkb extract` 只会处理 inventory 中同时满足以下条件的文件：
...
默认情况下，如果某个文件最近一次成功提取，且源文件大小和 mtime 没有变化，`pfkb extract` 会跳过该文件并记录 `up_to_date`。
...
```powershell
python -m pfkb extracts --inventory data/first-scan/inventory.sqlite --stats
python -m pfkb extracts --inventory data/first-scan/inventory.sqlite --status error
python -m pfkb extracts --inventory data/first-scan/inventory.sqlite --parser direct_text
python -m pfkb extracts --inventory data/first-scan/inventory.sqlite --json
```
